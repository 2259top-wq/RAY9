...
async function init() {
    try {
        ...
        els.downloadBtn.addEventListener('click', async () => {
            ...
            try {
                const res = await fetch(`${API_BASE}/download_missing`);
        ...
        const exportListDiv = document.getElementById('export-date-list');
        const exportSelectAllCb = document.getElementById('export-select-all');
        ...
        batchBtn.addEventListener('click', () => {
            ...
            availableDates.forEach(d => {
                ...
                cb.type = 'checkbox';
                cb.value = d;
                cb.className = 'export-date-cb';
                lbl.appendChild(cb);
                lbl.appendChild(document.createTextNode(' ' + d));
                exportListDiv.appendChild(lbl);
            });
            exportSelectAllCb.checked = false;
            exportModal.style.pointerEvents = 'auto';
            exportModal.style.opacity = '1';
        });

        exportSelectAllCb.addEventListener('change', (e) => {
            const cbs = document.querySelectorAll('.export-date-cb');
        ...
        exportConfirmBtn.addEventListener('click', async () => {
            ...
            const spin = batchBtn.querySelector('.spinner');
            ...
            try {
                ...
                for (let i = 0; i < selectedDates.length; i++) {
                    ...
                    els.dateSelect.value = d;
                    
                    // Update valid params for this date just to be safe
                    await updateValidParams();
                    
                    // Try to query
                    await handleQuery();
                    
                    // Wait for chart rendering (allow browser to paint)
                    await new Promise(r => setTimeout(r, 800));
                    
                    // Download day chart if visible
                    if (priceChartDay && currentDataCache) {
                        const canvas = priceChartDay.takeScreenshot();
                        const link = document.createElement('a');
                        const mode = els.displayModeSelect.value;
                        link.download = `Chart_${d}_${els.contractSelect.value}_${mode}_${els.strikeSelect.value}.png`;
                        link.href = canvas.toDataURL('image/png');
                        link.click();
                        // Pause briefly before downloading next to avoid browser block
...
// --- Batch Export Logic ---
const exportModal = document.getElementById('export-modal');
const btnBatchExport = document.getElementById('batch-export-btn');
const exportDateList = document.getElementById('export-date-list');
const btnExportConfirm = document.getElementById('export-confirm-btn');
const btnExportCancel = document.getElementById('export-cancel-btn');
const checkboxSelectAll = document.getElementById('export-select-all');

btnBatchExport.addEventListener('click', () => {
    exportDateList.innerHTML = '';
    const dates = Array.from(els.dateSelect.options).map(o => ({ value: o.value, text: o.text }));
    ...
    dates.forEach(d => {
        ...
        div.style.gap = '8px';
        div.innerHTML = `<input type="checkbox" value="${d.value}" class="export-date-checkbox" checked> <label>${d.text}</label>`;
        exportDateList.appendChild(div);
    });
...
checkboxSelectAll.addEventListener('change', (e) => {
    document.querySelectorAll('.export-date-checkbox').forEach(cb => {
...
btnExportConfirm.addEventListener('click', async () => {
    ...
    for (const dateVal of selectedDates) {
        els.dateSelect.value = dateVal;
        await fetchData(); // Fetch data for this date
    ...
    // Restore
    els.dateSelect.value = originalDate;
    await fetchData();
